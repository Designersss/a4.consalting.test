...
const Card = ({view} : {view?: ViewTypeForCard}) => {
    ...
    return (
        <div className={clsx("w-full h-full relative cursor-pointer bg-card-bg border-2 border-[#484D4E]", view === "large" ? "col-span-3 rounded-[34px] py-[30px] pr-[80px]" : "px-[18px] rounded-[40px] pt-[70px]")}>
            ...
            <div className={clsx("flex gap-10 items-center justify-end", view !== "large" && "flex-col")}>
                <div className={"flex flex-col items-center"}>
                    <span className="text-[26px] font-medium">3 месяца</span>
                    <div className={clsx("flex flex-col items-end", view === "large" ? "mt-4" : "mt-[30px]")}>
                        <div className={clsx("text-[50px] font-semibold", view === "large" && "text-default-color-accent")}>1990 ₽</div>
                        <div className="text-prev-price-card line-through text-[24px]">3990 ₽</div>
                    </div>
                </div>
